...
Intuitively, hints are partial types. They are built like types, except that there's one additional way to make hints: $\Hole$, which can be read as ``unknown'', ``hole'' or ``whatever''.
...
This order can be intuitively interpreted as an information increase: $\hintorder{H_1}{H_2}$ means that hint $H_2$ is more informative than $H_1$, but in a compatible way. In other words, $H_1$ and $H_2$ have the same structure, but some $\Hole$s from $H_1$ can be refined to something more informative in $H_2$.
...
\begin{center}
  $\infrule{\sidecond{\combinehints{H}{(\Fun{\Hole}{\Hole})} = \Fun{A}{H'}} \quad \fullhinting{\extend{\Gamma}{x}{A}}{e}{H'}{B}}{\hinting{\fun{x}{e}}{H}{\Fun{A}{B}}}$
  ...
  $\infrule{\sidecond{\combinehints{H}{(\Prod{\Hole}{\Hole})} = \Prod{H_1}{H_2}} \quad \hinting{a}{H_1}{A} \quad \hinting{b}{H_2}{B}}{\hinting{\pair{a}{b}}{H}{\Prod{A}{B}}}$
...
\begin{center}
  $\infrule{\sidecond{\combinehints{H}{(\Sum{\Hole}{\Hole})} = \Sum{H'}{B}} \quad \hinting{e}{H'}{A}}{\hinting{\inl{e}}{H}{\Sum{A}{B}}}$

  \vspace{2em}

  $\infrule{\sidecond{\combinehints{H}{(\Sum{\Hole}{\Hole})} = \Sum{A}{H'}} \quad \hinting{e}{H'}{B}}{\hinting{\inr{e}}{H}{\Sum{A}{B}}}$
...
\begin{center}
  $\hintfor{H}{\fun{x}{e}} = \combinehints{H}{(\Fun{\Hole}{\Hole})}$ \\
  $\hintfor{H}{\pair{e_1}{e_2}} = \combinehints{H}{(\Prod{\Hole}{\Hole})}$ \\
  $\hintfor{H}{\inl{e}} = \combinehints{H}{(\Sum{\Hole}{\Hole})}$ \\
  $\hintfor{H}{\inr{e}} = \combinehints{H}{(\Sum{\Hole}{\Hole})}$ \\
...
\begin{frame}{Hinting v2 -- alternative basic rules}

\begin{center}
  $\infrule[HintFor']{\hinting{e}{\hintfor{?}{e}}{A} \quad \sidecond{e\ \texttt{constructor}}}{\hinting{e}{\Hole}{A}}$
\end{center}
